...
|---|---|---|
| **Traitements retenus** | **Réduire** partout (dominant), + **Transférer** pour **R-05** (responsabilité *financière* des fonds vers PayFlow ; DPA = obligation RGPD) et **R-09** (cyber-assurance perte d'exploitation). **Aucun « éviter », aucun « accepter »** — tous les risques retenus ont un résiduel acté. | Analyste · 2026-09-24 |
| **R-13 retenu** (rejet du 23/09 reconsidéré) | Le rejet initial (essai du circuit de validation) **ne constitue pas une décision de gestion** : la non-conformité RGPD est un état de fait → R-13 est **retenu** au registre, traitement **Réduire** (registre art. 30, mentions légales, double opt-in avec preuve, droit à l'oubli, référent RGPD), résiduel **Faible** accepté jusqu'à l'échéance **déc. 2026**, ré-examen à chaque évolution réglementaire ou plainte/contrôle CNIL. | Analyste · 2026-09-24 |
| **R-14 modifié** | Directive analyste : probabilité inchangée (Moyenne), impact **Moyen → Élevé** (perte définitive de commandes + données clients, incident réel de 2025), niveau **Moyen → Élevé**, résiduel **Faible → Moyen** (conservateur). Modifiable à la prochaine revue. | Analyste · 2026-09-24 |
...
> ℹ️ **Niveaux de référence** : le niveau qui fait foi est celui de la **matrice probabilité × impact** (`04-evaluation.md` § 2) ; le rang DREAD sert d'ordre de traitement.
...
flowchart TB
    ...
    subgraph MEN["🟧 Menaces — registre validé (étapes 3–4)"]
        ...
        M4["R-12 · Divulgation données pers.<br/>LINDDUN-Disclosure · 🟠 ÉLEVÉ"]
    ...
    GF -.-> M4
    GF -.-> M5
    VAL["👤 Validation humaine :<br/>valide_par · décisions écrites<br/>registre validé le 2026-09-24"] -.-> T1
    VAL -.-> T2
    VAL -.-> T3
    VAL -.-> T4
    VAL -.-> T5
